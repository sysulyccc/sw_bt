...
class BaseGraph:
    ...
    def get_instance_with_graph_parameters(graph_type: str = None, **kwargs):
        ...
        :return:
        """
        # Map custom graph types to plotly types
        if graph_type == "ScatterGraph":
            return go.Scatter(**kwargs)
        elif graph_type == "BarGraph":
            return go.Bar(**kwargs)
        else:
            # Fallback to plotly.graph_objs
    ...
    def show_graph_in_notebook(figure_list: Iterable[go.Figure] = None):
        """
        Not used in our standalone version, but kept for compatibility
        :param figure_list:
        :return:
        """
        pass
    ...
    def _init_figure(self):
        ...
        for column_name, column_map in self._sub_graph_data:
            if isinstance(column_name, go.Figure):
                ...
                # Create graph object directly instead of using BaseGraph
                _graph_obj = ScatterGraph(
...
def report_graph(report_df: pd.DataFrame) -> [list, tuple]:
    ...
    :return: return **plotly.graph_objs.Figure** list.
    """
